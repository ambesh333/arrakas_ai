...
import { saveChatToStorage, loadChatFromStorage } from "@/utils/localStorage";
...
export default function ChatBox() {
  ...
  const missariEnabled = useSelector(
    (state: RootState) => state.chat.missariEnabled
  );
  const mutation = useChatMutation(publicKey, threadId, missariEnabled);

  useEffect(() => {
    const { messages: storedMessages, threadId: storedThreadId } =
      loadChatFromStorage();
    if (storedMessages.length > 0) {
      dispatch(initializeMessages(storedMessages));
      if (storedThreadId) {
        dispatch(setThreadId(storedThreadId));
      }
    }
  }, [dispatch]);
  ...
  useEffect(() => {
    if (messages.length > 0) {
      saveChatToStorage(messages, threadId);
    }
  }, [messages, threadId]);
